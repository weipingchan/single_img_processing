...
%Img_directory:The directory storaging the linearized tiff images
%Code_directory: The directory of library
%Result_directory: The directory to save results
%template='Phylogeny_10_2_ventral'; %The common name of a set of images
...
%if size(legacy,2)==1 legacy=legacy{1};, end;
...
%scaleevidence='forensic-evidence-labels.jpg';

%The information of references (These values are derived from the references provider)
reflectanceBlack = 1.1192;
reflectanceWhite = 99.1508;

%Define the file name which stores the label of each drawer
%labelfileName='Phylogeny 5_24_17 barcodes.csv';
%legacy=0; % 0 represents no, 1 represents yes (legacy indicates those images taken before March 2018)
legacy=str2num(legacy_input);
pauseornot=0; % 0 represents off, 1 represents on
%manual=0; % 0 represents off, 1 represents on (manual indicates if manually defined specimen boxes are used or not)
