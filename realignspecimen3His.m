...
for in=1:geolen
    ...
    for im=3:4
        ...
        for treat=1:3        
            ...
            elseif treat==2
                movingAdj =adapthisteq(imadjust(grayImg(movingin),[0, 2*avg],[],1.2)); %Adjust the image for registration only 
            else
            ...
            WimgOverlap=checkImgOverlapHis(fixedin,movingRegisteredAffineWithIC);
    ...
    for im=5:imgn-2
        ...
        for treat=1:3        
            if treat==1
                movingAdj = imadjust(grayImg(removeShadowHisRGB(imadjustn(movingin),0.7))); %Adjust the image for registration only 
            elseif treat==2
                movingAdj =imadjust(grayImg(imadjustn(movingin))); %Adjust the image for registration only 
            else
                nColors = 3;
            ...
            WimgDiffDist=checkImgDiffDist(movingin,imadjustn(movingRegisteredAffineWithIC));
            WimgOverlap=checkImgOverlapHis(fixedin,imadjustn(movingRegisteredAffineWithIC));
            SingleBandOverlap{treat,1}=WimgOverlap;
